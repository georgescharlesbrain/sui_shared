/// Utility functions to loop over a VecMap and send funds to addresses
module gotbeef::transfers
{
    ...
    {
        // Accumulate balance
        let total_balance = balance::zero(); //TODO Do we still need to use balance, can't this be done using the higher level coin module?
...
module gotbeef::transfers_tests
{
    ...
    fun test_send_all()
    {
        // initial transaction executed by SOMEONE
        ...
        // check if ADDR_3 received all the funds
    ...
    fun test_refund_all()
    {
        ...
        // check if ADDR_1 received the funds
        ts::next_tx(scen, ADDR_1); {
            let coin = ts::take_from_sender<Coin<SUI>>(scen);
            assert!( coin::value(&coin) == AMOUNT , 0 );
            ts::return_to_sender(scen, coin);
        };
        // check if ADDR_2 received the funds
        ts::next_tx(scen, ADDR_2); {
            let coin = ts::take_from_sender<Coin<SUI>>(scen);
            assert!( coin::value(&coin) == AMOUNT , 0 );
            ts::return_to_sender(scen, coin);
        };
        // check if ADDR_3 received the funds
        ts::next_tx(scen, ADDR_3); {
            let coin = ts::take_from_sender<Coin<SUI>>(scen);
            assert!( coin::value(&coin) == AMOUNT , 0 );
            ts::return_to_sender(scen, coin);
