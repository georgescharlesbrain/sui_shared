...
module polymedia_profile::profile
{
    ...
    /* getter functions */
    public fun registry_name(registry: &Registry): &String {
        &registry.name
    }
...
module polymedia_profile::profile_tests{
    ...
    use polymedia_profile::profile::{Self, Registry, Profile};


    //
    const ADMIN: address = @0x777; // registry owner
    const REGISTRY_NAME: vector<u8> = b"Polymedia Profiles test registry";
    ...
    fun test_end_end(){
        ...
        let scen_val = ts::begin(ADMIN);
        let scen = &mut scen_val;

        {
            profile::create_registry( REGISTRY_NAME , ts::ctx(scen));
        };

        ts::next_tx(scen, SOMEONE); {
            let registry_reference = ts::take_shared<Registry>(scen);
            let registry = &mut registry_reference;

            assert!( profile::registry_name(registry) == &string::utf8(REGISTRY_NAME), 0 );

            ts::return_shared(registry_reference);

        };

        // Create a profile
        ts::next_tx(scen, USER1); {
            let registry_reference = ts::take_shared<Registry>(scen);
            let registry = &mut registry_reference;
        ...
        // // Edit a profile
        // ts::next_tx(scen, USER1); {
        //     let profile_reference = ts::take_shared<Profile>(scen);
        //     let profile = &mut profile_reference;

        //     // // TODO make it optional to change values,
        //     // profile::edit_profile(
        //     //     profile,
        //     //     b"Profile User 1 adj",
        //     //     b"https://example.com/image_new.png",
        //     //     b"User 1 description new",
        //     //     b"User 1 data new",
        //     //     ts::ctx(scen),
        //     // );
        //     ts::return_shared(profile_reference);
        // };
